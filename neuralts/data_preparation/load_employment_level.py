...
def load_employment_level_data(input_path: str) -> pd.DataFrame:
    ...
    df['Employment_Level_Relative_Change'] = np.round( df['Employment_Level'].pct_change().bfill(), 5)
    ...
    df['Employment_Level_YoY_Relative_Change'] = np.round( df['Employment_Level'].pct_change(12).bfill(), 5)

    # Calculate 3-month moving average of employment level
    df['Employment_Level_MA_3'] = np.round(df['Employment_Level'].rolling(window=3).mean().bfill(), 5)

    # Calculate 6-month moving average of employment level
    df['Employment_Level_MA_6'] = np.round(df['Employment_Level'].rolling(window=6).mean().bfill(), 5)    
...
if __name__ == "__main__":
    ...
    input_path = os.path.join(os.getcwd(), "data", "raw", "bundesbank", "employment_level_germany.csv")
    output_path = os.path.join(os.getcwd(), "data", "processed", "historical_employment_level_germany.parquet")
